    ...
    >
      ...
      >
        <v-row>
          ...
          >
            <app-autocomplete
              v-model="book.trackDay"
              :items="trackDaysList"
              :rules="validate('Track Day')"
              :multiple="false"
              :chips="false"
              item-value="value"
              item-text="text"
              label="Select Track Day"
              @submit="onSave"
            />
          </v-col>
  ...
  export default {
    ...
    computed: {
      ...
      trackDaysList() {
        ...
        for (const trackDay of this.trackDays) {
          result.push({
            value: trackDay.id,
            text: trackDay.name + ' - ' + trackDay.track_date.date,
            disabled: (trackDay.left <= 0),
          })
        }
        return result
      },
      ...
      instalments() {
        let price = this.trackDayById(this.book.trackDay).price
        return [
    ...
    methods: {
      ...
      validate(field) {
        ...
        switch (field) {
          ...
          case 'Track Day': r = ['require']; break
          default: r = []; o = {}
        }
        ...
        if (this.hasPersonalDetails) {
          this.usersCardDetails({ userId: this.book.customer }).then(data => {
            if (data) {
              ...
              RealexHpp.lightbox.init("addCardLightTrackDay", responseEndpoint, response)
            }
          })
        }
      },
      ...
      onSave() {
        if (this.$refs.form.validate()) {
          ...
          /* if (this.book.card != '' && this.book.card > 0) { */
            this.trackDaysBookCreate(this.book).then(data => {
