  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        options: state => state.trackDays.options,
      }),
    ...
    mounted() {
      const trackDayId = this.$route.params.trackDayId
      this.selectedCard = this.options.card

      this.usersCardDetails({ userId: this.customer.id }).then(data => {
        if (data) {
          const response = JSON.parse(data)
          const responseEndpoint = `${window.location.protocol}//${window.location.host}/track-day/${trackDayId}`
